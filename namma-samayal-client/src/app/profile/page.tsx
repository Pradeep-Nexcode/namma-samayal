...
export default function ProfilePage() {
  ...
  if (loading) {
    return (
      <main className="w-full mt-32 mb-12 px-4 lg:px-6">
        <div className="mx-auto flex max-w-7xl items-center justify-center rounded-2xl bg-white shadow-sm border border-slate-200 p-12 min-h-[400px]">
  ...
  if (!user) {
    return (
      <main className="w-full mt-32 mb-12 px-4 lg:px-6">
        <div className="mx-auto flex flex-col items-center justify-center max-w-7xl rounded-2xl bg-white shadow-sm border border-slate-200 p-12 min-h-[400px]">
          <p className="rounded-xl border border-red-500/30 bg-red-50 px-6 py-4 text-sm font-medium text-red-600 mb-6">
  ...
  return (
    <main className="w-full bg-white pb-12 pt-32 px-4 lg:px-6">
      <div className="mx-auto w-full max-w-7xl rounded-2xl bg-white shadow-sm border border-slate-200 overflow-hidden flex flex-col md:flex-row min-h-[600px]">
        {/* Left Sidebar (20-25%) */}
        <aside className="w-full md:w-72 shrink-0 border-b md:border-b-0 md:border-r border-slate-200 bg-slate-50/50 p-6 lg:p-8 flex flex-col gap-8">
          {/* Profile Summary */}
          <div className="flex flex-col items-center text-center gap-4">
            <div className="relative h-24 w-24 overflow-hidden rounded-full border-4 border-white shadow-sm bg-slate-100 flex items-center justify-center text-slate-400 font-bold text-3xl">
              {form.profileImage ? (
                <Image
        ...
        <div className="flex-1 p-6 lg:p-12">
              ...
              <form onSubmit={onSaveProfile} className="space-y-6">
                <div className="grid gap-6 sm:grid-cols-2">
                  <label className="space-y-2 block">
                    ...
                    <input
                      ...
                      className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium text-slate-900 outline-none transition-all placeholder:text-slate-400 focus:border-[#e74c3c]/50 focus:bg-white focus:ring-4 focus:ring-[#e74c3c]/10"
                    />
                  </label>
                ...
                <label className="space-y-2 block">
                  ...
                  <textarea
                    ...
                    className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium text-slate-900 outline-none transition-all placeholder:text-slate-400 focus:border-[#e74c3c]/50 focus:bg-white focus:ring-4 focus:ring-[#e74c3c]/10 resize-none"
                  />
                </label>
                ...
                <div className="flex items-center gap-4 pt-4 border-t border-slate-100">
                  ...
                  {profileMessage && (
                    <span className="text-sm font-semibold text-green-600 bg-green-50 px-4 py-2 rounded-lg">
                      {profileMessage}
                    </span>
                  )}
                </div>
          ...
          {activeTab === "favorites" && (
            <div className="animate-in fade-in duration-300">
              <h2 className="text-2xl font-bold text-slate-900 mb-2">Favorite Recipes</h2>
              <p className="text-sm font-medium text-slate-500 mb-8">
              ...
              {user.favoriteRecipes.length === 0 ? (
                <div className="text-center py-16 rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50">
                  ...
                  <p className="text-sm font-medium text-slate-500">No favorite recipes yet.</p>
                </div>
              ) : (
                <ul className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                  {user.favoriteRecipes.map((item) => (
                    <li
                      key={recipeId(item)}
                      className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm hover:shadow-md transition-all group flex flex-col justify-between"
                    >
                      <div className="mb-6">
                        {recipeSlug(item) ? (
                          <Link
                            href={recipeHref(item)}
                            className="text-base font-bold text-slate-900 hover:text-[#e74c3c] line-clamp-2 transition-colors leading-tight"
                          >
                            {recipeLabel(item)}
                          </Link>
                        ) : (
                          <span className="text-base font-bold text-slate-900 line-clamp-2 leading-tight">
                            {recipeLabel(item)}
                          </span>
                        )}
                      </div>
                      <button
                        ...
                        className="self-end text-xs font-bold uppercase tracking-wider text-slate-400 hover:text-[#e74c3c] disabled:opacity-50 transition-colors"
                      >
                        Remove
                      </button>
                    </li>
          ...
          {activeTab === "saved" && (
            <div className="animate-in fade-in duration-300">
              <h2 className="text-2xl font-bold text-slate-900 mb-2">Saved Recipes</h2>
              <p className="text-sm font-medium text-slate-500 mb-8">
              ...
              {user.savedRecipes.length === 0 ? (
                <div className="text-center py-16 rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50">
                  ...
                  <p className="text-sm font-medium text-slate-500">No saved recipes.</p>
                </div>
              ) : (
                <ul className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                  {user.savedRecipes.map((item) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        disabled={isMutatingRecipe === recipeId(item)}
                        className="self-end text-xs font-bold uppercase tracking-wider text-slate-400 hover:text-amber-500 disabled:opacity-50 transition-colors"
                      >
                        Unsave
                      </button>
                    </li>
          ...
          {activeTab === "created" && (
            <div className="animate-in fade-in duration-300">
              <h2 className="text-2xl font-bold text-slate-900 mb-2">Created Recipes</h2>
              <p className="text-sm font-medium text-slate-500 mb-8">
              ...
              {user.createdRecipes.length === 0 ? (
                <div className="text-center py-16 rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50">
                  ...
                  <p className="text-sm font-medium text-slate-500 mb-4">No created recipes yet.</p>
                  <Link
                    href="/recipes/create"
                ...
                <ul className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                  {user.createdRecipes.map((item) => (
                    <li
                      key={recipeId(item)}
                      className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm hover:border-[#e74c3c]/20 transition-all flex flex-col justify-center min-h-[100px]"
                    >
                      {recipeSlug(item) ? (
                        <Link
                          href={recipeHref(item)}
